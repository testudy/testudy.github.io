...
但大部分URL都遵循通用的URL语法，大多数URL方案的URL语法都建立在这个由9部分构成的通用格式上
...
方案访问服务器以获取资源时要使用哪种协议无
用户某些方案访问资源时需要的用户名匿名

密码用户名后面可能要包含的密码，中间由冒号〈E-mail
旧分隔地址〉

主机资源宿主服务器的主机名或点分IP地址无
端口资源宿主服务器正在监听的端口号。很多方每个方案
案都有默认端口号(HTTP的默认端口号为80)特有
服务器上资源的本地名，由一个斜杠(n
将其与前面的URL组件分隔开来。路径组件的语
路径法是与服务器和方案有关的(本章稍后会讲到

URL路径可以分为若干个段，每段都可以有其特
有的组件。
某些方案会用这个组件来指定输入参数。参
数为名门直对。URL中可以包含多个参数字段，它
参数
们相互之间以及与路径的其余部分之间用分号
旧分隔
某些方案会用这个组件传递参数以激活应用
程序(比如数据库、公告板、搜索引擎以及其他
查询
因特网网关)。查询组件的内容没有通用格式。
用字符。?。将其与URL的其余部分分隔开来

一小片或一部分资源的名字。引用对象时，
不会将frag字段传送给服务器；这个字段是在客
片段
户端内部使用的。通过字符。#。将其与URL的其余
部分分隔开来
